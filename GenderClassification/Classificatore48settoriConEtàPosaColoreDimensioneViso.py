...
array = np.genfromtxt ("file5finaleConEtàPosaColoreDimensioneViso.csv", delimiter = ",", skip_header = 1)
array = array.astype(int)

data = [[0 for y in range(52)] for x in range(array.shape[0])]
...
for i in array:
    data[count]= i[1:49]
    data[count]=np.append(data[count],i[51:])
    genere[count] = i[50]
    count = count + 1
