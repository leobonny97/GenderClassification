...
array = np.genfromtxt ("file6finaleConEtàPosaColoreDimensioneViso.csv", delimiter = ",", skip_header = 1)
array = array.astype(int)

data = [[0 for y in range(84)] for x in range(array.shape[0])]
...
for i in array:
    data[count]= i[1:81]
    data[count]=np.append(data[count],i[83:])
    genere[count] = i[82]
    count = count + 1
